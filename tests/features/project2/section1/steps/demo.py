...
from tests.features.project2 import project2_config as config
import tests.features.project2.section1.messages as messages
...
def step_impl(context, file_name):
    ...
    FileManager.copy_file(config.TEST_DATA_DEMO + 'P2_S1_DEMO_1.json', config.JSON_OUTPUT + 'P2_S1_DEMO_1.json')
...
def step_impl(context):
    ...
    if file_found_dir == expected_dir:
        LoggingManager.bdt_logger('INFO', context.file_id, messages.P2_S1_DEMO_1_EXPECTED, matched_file)
    elif file_found_dir in error_dirs:
        LoggingManager.bdt_logger('ERROR', context.file_id, messages.P2_S1_DEMO_1_ERROR, matched_file, open(matched_file).readline())
    else:
        LoggingManager.bdt_logger('ERROR', context.file_id, messages.P2_S1_DEMO_1_UNKNOWN)

    assert file_found_dir == expected_dir


@then('validates correctly against the schema {schema}')
def step_impl(context, schema):
    ...
    if schema_result == expected_schema_result:
        LoggingManager.bdt_logger('INFO', context.file_id, messages.P2_S1_DEMO_2_EXPECTED, context.matched_file)
    else:
        LoggingManager.bdt_logger('ERROR', context.file_id, messages.P2_S1_DEMO_2_ERROR, context.matched_file, schema_result)

    assert schema_result == expected_schema_result


@then('all key values match the expected values {expected_output}')
def step_impl(context, expected_output):
    ...
    if len(unresolved_conflicts) == 0:
        LoggingManager.bdt_logger('INFO', context.file_id, messages.P2_S1_DEMO_3_EXPECTED, context.matched_file)
    else:
        LoggingManager.bdt_logger('ERROR', context.file_id, messages.P2_S1_DEMO_3_ERROR, context.matched_file, str(unresolved_conflicts))
